...
from tensorflow.keras.layers import Reshape, Dense, LSTM, Dropout, Input, ConvLSTM2D, Conv1D, Flatten, MaxPooling1D, Embedding
...
def create_data(data_unscaled, start_train ,end_train ,n_windows,n_outputs):
    ...
    for i in range(start_train, end_train):
        ...
        y.append(data_unscaled[i-n_windows+1:i+1, 0:n_outputs+1])
...
HP_OUTPUT = hp.HParam('output_number',hp.Discrete(list(range(39,40))))
...
def train_model(run_dir,hparams):
    ...
    model = Sequential()
    model.add(Conv1D(100,kernel_size=10,padding='causal',activation='relu',input_shape=(50,40)))
    #model.add(MaxPooling1D(pool_size=2))
    model.add(Conv1D(10,kernel_size=10,padding='causal',activation='relu'))
    model.add(MaxPooling1D(pool_size=2))
    ...
    #model.fit(X_train[:, :, :], y_train[:, 49, :], epochs=10)
    ...
    model.fit(X_train, y_train[:,hparams[HP_WINDOW]-1,:], epochs=50, batch_size=batch_size, validation_data=(X_test, y_test[:,hparams[HP_WINDOW]-1,:]), callbacks=[
...
def test_model(hparams):
    ...
    tf.compat.v1.keras.backend.clear_session()
    model = load_model('model_' + str(hparams[HP_NUM_UNITS]) + '_' + str(hparams[HP_DROPOUT]) + '_' + str(hparams[HP_OPTIMIZER]) + '_' + str(hparams[HP_WINDOW]) + '_' + '39' + '.h5')
...
test_model({HP_NUM_UNITS: 100, HP_DROPOUT: round(0.2,1), HP_OPTIMIZER: 'rmsprop', HP_OUTPUT: 1, HP_WINDOW: 50, HP_HORIZON:[1,5,10,25,50]})
