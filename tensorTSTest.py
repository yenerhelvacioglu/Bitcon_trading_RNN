...
keep_prob = 0.95 #dropout keep probability rate
learning_rate = 0.001 #learning rate

layer_rnn_cell = [tf.compat.v1.nn.rnn_cell.GRUCell(num_units=n_neurons, activation=tf.nn.leaky_relu) for layer in range(n_layers)]
if not testing:
...
def create_batches(df, windows, input, output):
   ## Create X
        x_data = df[:-windows,0:n_inputs]
        X_batch = x_data.reshape(-1, windows, input)
    ## Create y
        y_data = df[windows:,0:n_outputs]
...
with tf.Session() as sess:
    if not testing:
        ...
        for iteration in range(n_iterations):
            ...
            rnn_outputs_val, state_val = sess.run([outputs, state], feed_dict={X:X_batch, y:y_batch})
            if iteration %10 == 0:
...
pd.DataFrame(y_test.reshape(-1)).to_csv('y_test.csv')
pd.DataFrame(y_pred.reshape(-1)).to_csv('y_pred.csv')

plt.title("Forecast vs Actual", fontsize=14)
plt.plot(pd.Series(np.ravel(y_test[:,0,:])), markersize=8, label="Actual", color='green')
plt.plot(pd.Series(np.ravel(y_pred[:,0,:])), markersize=8, label="Forecast", color='red')
