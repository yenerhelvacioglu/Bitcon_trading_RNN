...
def train_test_model(run_dir,hparams):
    ...
    model = Sequential()
    model.add(LSTM(hparams[HP_NUM_UNITS], return_sequences=True ,input_shape=(n_windows, n_inputs) ,activation='tanh' ,kernel_initializer='TruncatedNormal' ,bias_initializer=initializers.Constant(value=0.1), dropout=hparams[HP_DROPOUT] ,recurrent_dropout=hparams[HP_DROPOUT]))
    ...
    model.add(Dense(units=99, activation='linear'))
    weights = np.full([1,1,n_outputs],0.1)
    ...
    #model.fit(X_train, y_train[:, 0, :], epochs=100, batch_size=batch_size, validation_data=(X_test, y_test[:, 0, :]))
    ...
    model.fit(X_train, y_train[:,0,:], epochs=1000, batch_size=batch_size, validation_data=(X_test, y_test[:,0,:]), callbacks=[
    ...
    for iteration in range(n_windows):
        ...
        X_test_extended = np.concatenate((X_test_extended[:, -n_windows+1:, :], np.reshape(predicted, (X_test.shape[0], 1, X_test.shape[2]))), axis=1)
