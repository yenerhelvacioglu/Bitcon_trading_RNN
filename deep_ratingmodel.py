...
def create_data(data_scaled, start_train ,end_train ,n_windows):
    ...
    y_train = np.reshape(y_train, (y_train.shape[0], n_outputs))
...
def train_test_model(run_dir,hparams):
    hp.hparams(hparams)
    [X_train, y_train] = create_data(data_scaled=data.values, start_train=hparams[HP_WINDOW], end_train=end_train, n_windows=hparams[HP_WINDOW])
    [X_test, y_test] = create_data(data_scaled=data.values, start_train=end_train, end_train=end_test, n_windows=hparams[HP_WINDOW])
    #pd.DataFrame(sc.inverse_transform(np.reshape(X_test, ((end_test - end_train)*hparams[HP_WINDOW], n_inputs)))).to_csv('X.csv')
    #pd.DataFrame(sc.inverse_transform(np.reshape(y_test, ((end_test - end_train)*hparams[HP_WINDOW], n_outputs)))).to_csv('y.csv')
    pd.DataFrame(X_sc.inverse_transform(X_test)).to_csv('X.csv')
    pd.DataFrame(y_sc.inverse_transform(y_test)).to_csv('y.csv')
    tf.compat.v1.keras.backend.clear_session()
    model = Sequential()
    model.add(Dense(units=hparams[HP_NUM_UNITS], activation='relu', bias_initializer=initializers.Constant(value=0.1), input_shape=(n_inputs,)))
    ...
    #weights = np.full([1,1,n_outputs],0.5)
    #weights[0,0,0:57] = 1
